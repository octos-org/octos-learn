...
describe("LearningWorkspace", () => {
  ...
  it("refetches durable OLL artifacts when a voice turn completes", async () => {
    sessionFilesMock.getSessionFiles
      .mockResolvedValueOnce([])
      .mockResolvedValue([
        {
          filename: "completed-turn.octos-lesson.json",
          path: "study/oll/completed-turn.octos-lesson.json",
          size_bytes: 100,
          modified_at: "2026-08-03T14:06:46.000Z",
        },
      ]);
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ...
        close: { summary: "恢复完成", focus: ["completed-card"] },
      }),
    }));

    render(
      <LearningWorkspace
        sessionId="learn-turn-completed"
        voiceEnabled={false}
        onBack={vi.fn()}
      />,
    );
    ...
    act(() => {
      conversationMock.options?.onTurnComplete?.("completed-turn");
    });

    await waitFor(() => {
      expect(sessionFilesMock.getSessionFiles).toHaveBeenCalledTimes(2);
      expect(screen.getByText("本轮完成后发现的课程")).toBeTruthy();
    });
  });
