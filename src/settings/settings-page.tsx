...
export function AdminSettingsPage() {
  ...
  const isAdminOnlyTab = activeTab === "authentication";

  // Keep the settings rail searchable even though the standalone product
  // intentionally exposes only the controls needed by the learning canvas.
  ...
  const renderTabButton = ({ id, label, icon: Icon, adminOnly }: TabDef) => (
    ...
    >
      ...
      {label}
      {adminOnly && (
        <span className="ml-auto shrink-0 rounded-full bg-accent/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-accent">
          Admin
        </span>
      )}
    </button>
