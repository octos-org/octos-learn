...
> **文档状态：历史设计记录。** 本计划用于保留公网架构与决策过程，不是现行部署手册。
> 其中邀请制发布等早期选择已经被后续实现替代。当前行为以
> [公网部署手册](PUBLIC_DEPLOYMENT_RUNBOOK.md) 和
...
本计划最初面向受控评审、GOSIM 展示和小规模邀请测试，目标是先获得一条安全、稳定、可监控、可回滚的真实公网链路。之后产品已改为允许邮箱验证码自由注册；相关实现与运维规则见现行文档。
