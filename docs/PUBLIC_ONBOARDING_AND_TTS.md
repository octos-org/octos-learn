...
这里限制的是本站实际放行的请求字符数（含标点、空格，并为程序可能补上的末尾标点预留 1 个字符）。在向供应商发送前，SQLite 事务先记录额度；并发请求不能同时抢到同一份余额。请求失败、浏览器中断或超时不能证明供应商没有计费，因此不自动返还。同一用户使用相同音色配置重播完全相同的旁白时，会从持久化音频缓存读取，不再调用供应商，也不会重复计量。不同用户、音色或配置不会共享缓存键。
