...
- OLL 提交 `6c5cfb983a825216af7f96f20d9969e7d4a4496c` 已推送至远端分支 `codex/whiteboard-assistance`。它提供原子 AI 笔迹事务、稳定组件 ID、`origin: student | ai`、文档 v2、来源纳入校验的选区 v5，以及旧 v1-v4 兼容读取。旧写入器会拒绝新文档，避免悄悄丢掉消费记录。
...
- octos-learn：87 个测试文件、800 项测试通过；默认关闭与显式开启板书能力的生产构建均通过；lint 0 errors。安装态构建发现并补齐了新 Runtime 初始状态中的 `selection_transform_enabled: false`，证明移动默认锁定。真实 React 白板验证了板书出现、无卡片、显式移动、撤销只移除批注、刷新不复活。
- learning-coach：安装态 OLL contract 通过，144 项完整测试通过。21 次真实模型质量矩阵中 20 次成功，覆盖正确、错误、不确定识别、转写、改写、明确绘图和普通解释；保留 1 次达到原 30 秒截止的失败。
...
OLL 远端分支为 `codex/whiteboard-assistance`，提交为 `6c5cfb983a825216af7f96f20d9969e7d4a4496c`。learning-coach 本地分支 `codex/selection-intent-alignment` 的实现提交为 `3cd19d1`，精确 pin 提交为 `0f3e04b`。本文件所在的 octos-learn 分支包含产品接线、精确 pin、BOM 与验收证据。未执行合并、发布或生产功能开关变更；发布时应先上线兼容读取器，再开启 `VITE_ENABLE_BOARD_WRITING=true`。
