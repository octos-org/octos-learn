...
正式启用需要先发布支持 v0.3/v2/v5 的最低客户端，再开启输出；最低 OLL 实现已固定为远端提交 `6c5cfb983a825216af7f96f20d9969e7d4a4496c`，两个消费端的 lock、contract 与 BOM 均指向该提交。最终客户端最低提交以本分支合并后的 release SHA 为准。回滚保留该读取器，只关闭新输出，不能回滚到会丢失消费记录的旧实现。现有已生成板书在关闭新输出后仍按普通笔迹恢复。
